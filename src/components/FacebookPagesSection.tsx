...
const FacebookPagesSection = () => {
    ...
    const linkPage = async (pageIdRaw: string | number) => {
        const pageId = String(pageIdRaw);

        try {
            setLinking(true);
            await api.post(
                "/api/rest/social/link-facebook-page",
                pageId,
                { headers: { "Content-Type": "application/json" } }
            );
            ...
            setLinkedPageId(pageId);
            toast.success(
                language === "ka" ? "გვერდი დაკავშირდა!" : "Page linked!"
            );
        } catch (err) {
            console.error(err);
            toast.error(
    ...
    const fetchPages = async () => {
        try {
            ...
            const pagesRes = await api.get("/api/rest/social/get-facebook-pages");
            const pageList: FacebookPage[] = Array.isArray(pagesRes.data?.content)
                ? pagesRes.data.content
                : [];

            setPages(pageList);

            const accountsRes = await api.get("/api/rest/social/accounts");
            const fbAccount = accountsRes.data?.content?.find(
                (a: any) => a.platformId === 1
            );

            const existingLinkedPageId = normalizeId(fbAccount?.pageId);
            setLinkedPageId(existingLinkedPageId);


            if (!existingLinkedPageId && pageList.length === 1) {
                await linkPage(pageList[0].id);
            }
        } catch (err) {
            console.error(err);
            toast.error(
                language === "ka"
                    ? "Facebook გვერდების ჩატვირთვა ვერ მოხერხდა"
                    : "Facebook pages loading failed"
            );
        } finally {
            setLoading(false);
        }
    ...
    const unlinkPage = async () => {
        try {
            ...
            setLinkedPageId(null);
            toast.success(
                language === "ka" ? "გვერდი გათიშულია!" : "Page unlinked!"
            );
        } catch (err) {
            console.error(err);
            toast.error(
                language === "ka"
                    ? "გათიშვა ვერ მოხერხდა"
                    : "Failed to unlink page"
            );
        } finally {
            setUnlinking(false);
        }
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400">
                            {language === "ka"
                                ? "მართეთ თქვენი Facebook გვერდები"
            ...
            {pages.length > 0 && (
                <div className="space-y-3">
                    {pages.map((p) => {
                        ...
                        const isLinked = linkedPageId === pageId;
    ...
    <span className="text-green-400 text-sm font-semibold px-3 py-1 rounded-full bg-green-500/10">
      {language === "ka" ? "დაკავშირებულია" : "Connected"}
                                            ...
                                            <Button
                                                ...
                                                size="sm"
                                            >
                                                {language === "ka" ? "გვერდის გათიშვა" : "Unlink Page"}
                                            </Button>
                                        </>
                                    ) : (
                                        <Button
                                            onClick={() => linkPage(pageId)}
                                            disabled={linking}
                                            size="sm"
                                        >
                                            {language === "ka" ? "დაკავშირება" : "Link Page"}
            ...
            {pages.length > 1 && !linkedPageId && (
                <div className="mt-4 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-start gap-2">
                    <AlertCircle className="w-4 h-4 text-blue-400 mt-0.5" />
                    <p className="text-sm text-blue-300">
                        {language === "ka"
                            ? "აირჩიეთ Facebook გვერდი დასაკავშირებლად"
                            : "Select a Facebook page to link"}
                    </p>
                </div>
            )}
        </div>
