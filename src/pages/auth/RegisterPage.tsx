...
const RegisterPage: React.FC = () => {
  ...
  const businessTypes = [
    { id: 1, value: "restaurant", label: language === "ka" ? "რესტორანი/კაფე" : "Restaurant/Cafe" },
    { id: 2, value: "retail", label: language === "ka" ? "საცალო ვაჭრობა" : "Retail" },
    { id: 3, value: "services", label: language === "ka" ? "სერვისები" : "Services" },
    { id: 4, value: "tourism", label: language === "ka" ? "ტურიზმი" : "Tourism" },
    { id: 5, value: "freelancer", label: language === "ka" ? "ფრილანსერი" : "Freelancer" },
    { id: 6, value: "other", label: language === "ka" ? "სხვა" : "Other" },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      return toast.error(language === "ka" ? "პაროლები არ ემთხვევა" : "Passwords do not match");
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-foreground-muted">
          {language === "ka" ? "შექმენით თქვენი ანგარიში" : "Get started with SocialGenius"}
        </p>
      </div>
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        {/* First Name */}
        <div>
          <Label>{language === "ka" ? "სახელი" : "First Name"}</Label>
          <div className="relative mt-1">
            <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground-muted" />
            <Input
              name="firstName"
              className="input-elegant pl-10"
              placeholder={language === "ka" ? "შეიყვანეთ სახელი" : "Enter first name"}
        ...
        {/* Last Name */}
        <div>
          <Label>{language === "ka" ? "გვარი" : "Last Name"}</Label>
          <div className="relative mt-1">
            <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground-muted" />
            <Input
              name="lastName"
              className="input-elegant pl-10"
              placeholder={language === "ka" ? "შეიყვანეთ გვარი" : "Enter last name"}
        ...
        {/* Business Name */}
        <div>
          <Label>{language === "ka" ? "ბიზნესის სახელი" : "Business Name"}</Label>
          <div className="relative mt-1">
        ...
        {/* Business Type */}
        <div>
          <Label>{language === "ka" ? "ბიზნესის ტიპი" : "Business Type"}</Label>
          <Select onValueChange={(v) => {
            const sel = businessTypes.find(b => b.value === v);
            setFormData({ ...formData, businessId: sel?.id || 0 });
          }}>
            <SelectTrigger className="input-elegant h-12 px-4 mt-1">
              <SelectValue placeholder={language === "ka" ? "აირჩიეთ ტიპი" : "Select business type"} />
            </SelectTrigger>
            <SelectContent>
              {businessTypes.map((type) => (
        ...
        {/* Password */}
        <div>
          <Label>{language === "ka" ? "პაროლი" : "Password"}</Label>
          <div className="relative mt-1">
        ...
        {/* Confirm Password */}
        <div>
          <Label>{language === "ka" ? "გაიმეორეთ პაროლი" : "Confirm Password"}</Label>
          <div className="relative mt-1">
            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground-muted" />
            <Input
              name="confirmPassword"
        ...
        <Button type="submit" className="w-full btn-hero h-12 text-lg">
          {isLoading
            ? language === "ka" ? "რეგისტრაცია..." : "Creating..."
            : language === "ka" ? "რეგისტრაცია" : "Create Account"}
        </Button>
      ...
      <div className="text-center">
        <p className="text-foreground-muted">
          {language === "ka" ? "უკვე გაქვთ ანგარიში?" : "Already have an account?"}{" "}
          <Link to="/login" className="text-primary-light hover:text-accent">
            {language === "ka" ? "შესვლა" : "Sign in"}
          </Link>
        </p>
      </div>
